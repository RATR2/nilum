...
public final class NilumTcpAssetServer {
    ...
    public static void serve(Socket socket, Function<String, byte[]> assetLookup) {
        Thread thread = new Thread(() -> loop(socket, assetLookup), "nilum-tcp-asset-server");
        thread.setDaemon(true);
        thread.start();
    }

    private static void loop(Socket socket, Function<String, byte[]> assetLookup) {
        try {
            while (!socket.isClosed()) {
                AssetRequestPacket request = AssetRequestPacket.decode(
                        NilumTcpFrames.readFrame(socket.getInputStream()));

                byte[] data = assetLookup.apply(request.assetId());
                AssetDataPacket response = data != null
                        ? new AssetDataPacket(request.assetId(), true, data)
                        : new AssetDataPacket(request.assetId(), false, new byte[0]);
