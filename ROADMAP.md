...
- [x] Reorganized nilum-builds output by role instead of by Gradle module name, each a flat folder of jars (the filename's own SERVER-FABRIC/CLIENT-NEOFORGE/etc. suffix says what it is, no need to nest that same distinction into a subfolder too): `nilum-server/` (the full per-loader jars), `nilum-universal/` (Fabric + NeoForge + Paper merged into one jar that works no matter which loader it's dropped into, root `mergeUniversalJar` task), `nilum-client/` (the per-loader `clientOnlyJar` outputs plus those same two merged into one `MULTILOADER` jar, root `mergeMultiloaderJar` task), `nilum-source/` (nilum-common/-server/-client). The three-loader merge works because Fabric mods, NeoForge mods, and Paper plugins each look for a differently-named manifest at a fixed path (`fabric.mod.json`, `META-INF/neoforge.mods.toml`, `plugin.yml`) inside the same jar and otherwise ignore files they don't recognize; Fabric's own contribution has to merge in first, since its `META-INF/MANIFEST.MF` carries Loom/Mixin attributes the other two don't need and `DuplicatesStrategy.EXCLUDE` keeps whichever came first. `nilum-api` is no longer copied into nilum-builds at all; it publishes as a GitHub Release (`v<version>`, `gh release create` in `build-and-publish-branch.sh`) on the nilum repo itself instead, since it's a public compile-time dependency for third-party developers, not a runnable artifact
